...
async def run_setup_agent(repo_dir: str, api_key: str, model: str = None, log_callback = None) -> str:
    ...
    system_instructions = (
        ...
        "STEPS:\n"
        "1. Explore the repository (using e2b_grep_search and run_command('ls')) to find "
        ...
        "4. Use the `run_command` tool to install dependencies (e.g., `npm install`, `pip install -r requirements.txt`).\n"
        "5. Use the `run_command` tool to run the build or tests to verify the setup.\n"
        "6. If a command fails because of missing system dependencies (like `make` or `gcc`), "
        "use `run_command` to install them via `apt-get update && apt-get install -y ...`.\n\n"
